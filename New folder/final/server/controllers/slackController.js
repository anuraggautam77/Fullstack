...
const slackController = {
  ...
  getMessagesBy : async (req, res, next) => {
    try {
      ...
      const condition = {};
      if (req.params.value) {
        condition.channelId = req.params.value;
        condition.workspaceName = req.params.workspaceId;
      } else {
    ...
    try {
      ...
      if (req.params.channelid) {
        ...
        if (uStatus.nModified) {
          const lists = await Channel.findOne({ channelId: req.params.channelid }).populate('members', 'name userId').exec();
