const kanbanData = require('../data/boards');
...
const kanbanController = {
  ...
  getOneBoard(req, res) {
    if (req.params.id) {
      if (kanbanData.boards.length > 0) {
        let boardlist = kanbanData.boards.find((obj) => {
          if (obj.boardId === parseInt(req.params.id)) {
            return obj;
          }
        });
        if (boardlist) {
  ...
  updateBoard(req, res) {
    ...
    if (req.params.id) {
      if (kanbanData.boards.length > 0) {
        const index = kanbanData.boards.findIndex((obj) => (obj.boardId === parseInt(req.params.id)));
